...
const productKeywords = {
  ...
  'adjustable-dumbbell-set-5-52-5-lbs': 'dumbbell,gym,fitness',
...
async function updateRelatedImages() {
  try {
    ...
    for (const product of products) {
      ...
      const imageUrl1 = `https://loremflickr.com/800/600/${encodeURIComponent(keywords)}?random=${product.id}`;
      const imageUrl2 = `https://loremflickr.com/800/600/${encodeURIComponent(keywords)}?random=${product.id}-2`;
    ...
    console.log('\n✅ All product images updated with relevant content using LoremFlickr!');
  } catch (err) {
